...
renders a preview either from a **template** or from a **command** you run. Use
this whenever a task involves turning links (PRs, tickets, dashboards) into
inline previews.
...
4. The rule's action renders a Block Kit attachment — a **`template`** (Go
   text/template) or a **`run`** command (JSON in → attachment JSON out). →
   `reference/actions.md`
5. Murtaugh posts all previews for the message in one `chat.unfurl` call.

## The two action modes (at a glance)
...
| Writing a `template` or a `run` command (stdin/stdout contracts) | `reference/actions.md` |
